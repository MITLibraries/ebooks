...
def s3_conn():
    with mock_aws():
        ...
        conn.put_object(
            Bucket="samples", Key="has_volumes_a_2009.txt", Body="Vol 2009a"
        )
        conn.put_object(
            Bucket="samples", Key="has_volumes_b_2009.txt", Body="Vol 2009b"
        )
        conn.put_object(Bucket="samples", Key="sample.mp4", Body="fixtures/sample.mp4")
        yield conn
...
def alma(record, serial):
    with requests_mock.Mocker() as m:
        m.get(
            "https://mock.com/sample?apikey=", status_code=200, content=record.encode()
        )
        m.get(
            "https://mock.com/serial?apikey=", status_code=200, content=serial.encode()
        )
        m.get(
            "https://mock.com/fake_item?apikey=",
            status_code=200,
            content=(
