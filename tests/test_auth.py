...
def test_is_safe_url_is_safe(testapp):
    x = is_safe_url('http://localhost/?safe')
    assert x is True


def test_is_safe_url_is_not_safe(testapp):
...
@patch.dict(os.environ, {
    ...
    'IDP_CERT': 'iamacert'})
def test_load_saml_settings_returns_json():
    x = load_saml_settings()
    assert x['sp']['entityId'] == 'http://ebooks.test/shibboleth/'


@patch.dict(os.environ, {'SP_ENTITY_ID': ''})
def test_load_saml_settings_missing_env_variable_raises_exception():
...
def test_prepare_flask_request(testapp):
